...
import { Box, Button, Card, Typography, Backdrop, CircularProgress, IconButton } from '@mui/material';
...
export const Launch = (props: unknown) => {
  const context = useContext(FavouritesContext);
  const { id } = useParams<string>()
  const [imageIdx, setImageIdx] = useState<number>(0)
  const [{ fetching, data, error }] = useQuery({query: launchQuery(id), pause: !id});
  ...
  const addToFavourites = (e: any):void => {
    if(context.dispatch !== null) {
      context.dispatch({type: ADD_TO_FAVOURITES, payload: data.launch.id as string})
    }
  }
  const removeFromFavourites = (e: any):void => {
    if(context.dispatch !== null) {
      context.dispatch({type: REMOVE_FROM_FAVOURITES, payload: data.launch.id as string})
    }
  }
      ...
      <Card className="launch">
        <div className="row">
          <div className="launch-card launch-card--big">
            {
              data.launch.links?.flickr_images && data.launch.links?.flickr_images.length > 0 && data.launch.mission_name ? (
                <div className={"launch-card__img-container"}>
                  <img className={"launch-card__img"} src={data.launch.links?.flickr_images[imageIdx] as string} alt={data.launch.mission_name} />
                  <div className={"buttons-bar"}>
                    ...
                    <div
                      ...
                      {} : {display: "none"}}
                      className={"d-flex justify-center launch-card__img-slider-buttons"}
                    >
                      <Button
                        variant={"contained"}
                        color={"secondary"}
            ...
            <div className={"launch-card__container"}>
              ...
              >
                ...
                </Box>
                {
                  data.launch.launch_date_utc ?
                    new Date(Date.parse(data.launch.launch_date_utc)).toLocaleString() :
                    "no date found"
                }
              </Typography>
              ...
              >
                {
                  data.launch.launch_site && data.launch.links && data.launch.links.article_link ? (
